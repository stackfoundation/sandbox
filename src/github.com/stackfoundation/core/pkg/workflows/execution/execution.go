...
func handleChangeAndTransitionNext(e Execution, w *v1.Workflow, c *v1.Change) error {
	context := NewContext(w, c)
...
func Execute(e Execution, w *v1.Workflow) error {
	if len(w.Spec.State.Changes) < 1 {
		return e.TransitionNext(&Context{Workflow: w}, initialTransition)
	}
